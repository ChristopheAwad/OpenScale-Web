...
<div class="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
	...
	<nav class="fixed bottom-0 left-0 right-0 bg-surface-100/80 backdrop-blur-xl border-t border-white/5 flex justify-around items-center py-3 px-4 z-50 shadow-2xl shadow-black/50">
		{#each navItems as item}
			{@const isActive = $page.url.pathname === item.href}
			<a
				href={item.href}
				class="flex flex-col items-center gap-1 p-2 rounded-[var(--radius)] transition-all {isActive ? 'text-primary-400 bg-primary-500/10' : 'text-white/50 hover:text-white hover:bg-white/5'}"
			>
